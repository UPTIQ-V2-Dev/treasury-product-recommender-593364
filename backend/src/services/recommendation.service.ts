...
const submitContactRequest = async (
    ...
    for (const analysis of analyses) {
        if (analysis.recommendations && Array.isArray(analysis.recommendations)) {
            const recommendation = (analysis.recommendations as any[]).find((rec: any) => rec.id === recommendationId);
            if (recommendation) {
 ...
 */
const validateRecommendationAccess = async (
    recommendationId: string,
    userId: number
): Promise<{ analysisId: string; recommendation: any }> => {
    // Find analysis results for the user that might contain this recommendation ID
    const analyses = await prisma.analysisResult.findMany({
        where: {
            userId,
